...
namespace ikanopu.Core {
    ...
    public static class ImageUtil {
        ...
        public static void DrawCropPreview(this Mat mat, IEnumerable<(CropOption.Team, Rect)> src, IEnumerable<(int, string)> recognized) {
            ...
            foreach (var ((team, rect), i) in src.Select((x, i) => (x, i))) {
                ...
                if (recs.ContainsKey(i)) {
                    mat.PutTextExtra(font, brush, rect.Right, rect.Top, $"[{i}]: {recs[i]}");
                } else {
                    mat.PutTextExtra(font, brush, rect.Right, rect.Top, $"[{i}]: [未登録/未検出]");
                }
            }
        }
        /// <summary>
        ...
        public static RecognizeResult Recognize(this (CropOption.Team, Mat)[] postMats, GlobalConfig config) {
                ...
                config.RegisterUsers
                      .Select(user => {
                          ...
                          return (user, computeDatas.Select(target => {
                              ...
                              //Console.WriteLine($"name:{user.DisplayName} fromKP:{kp.Length} targetKP:{target.KeyPoints.Length} matches:{matches.Length} distance:{distance} kpDiff:{keypointDiff}, score:{score}");
                              return new {
                                  // 元データ
                                  Team = target.Team,
                                  Image = target.Image,
            ...
            var recognizedUsers =
                matchResults.Select(r => {
                    ...
                    return new {
                        ...
                        KeyPointDiff = detect.Value.KeyPointDiff,
                    };
                })
                .Where(x => x != null)
                .ToArray();
            ...
            // 同一だった場合にもKeyPointの差分に応じてペナルティを増やす
            var filteredUsers =
                    ...
                    recognizedUsers.Where(x => !filteredUsers.Contains(x)) // 現在ない中で
                                   ...
                                   .Select(x => {
                                       ...
                                       return new {
                                           ...
                                           KeyPointDiff = x.KeyPointDiff,
                                       };
                                   }).ToArray();
                // ここでもかぶったら、Independencyが高い方を採用するしかないじゃない
                var addUsers =
                    diffUsers.GroupBy(x => x.Index)
                             .Select(x => x.OrderByDescending(y => y.Independency).First())
                             .ToArray();
                filteredUsers.AddRange(addUsers);
            }
            #endregion
